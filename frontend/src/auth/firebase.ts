// Google sign-in via Firebase — additive to email+password login (see
// backend/app/auth/router.py's google_login docstring). Firebase only
// handles proving "this really is this Google account"; the resulting ID
// token is exchanged once for this app's own JWT pair at
// /api/v1/auth/google, so every other part of the app (permission checks,
// token refresh, RLS scope) stays on the existing JWT machinery unchanged.
